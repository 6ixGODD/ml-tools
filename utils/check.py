...
def check_data(data):
    ...
    if data.columns[0] != "label":
        raise ValueError("The first column of data should be label.")
...
def check_cfg(cfg):
    ...
    if not isinstance(cfg.feature_selection['method'], str):
        raise ValueError("Feature selection method should be a string.")
